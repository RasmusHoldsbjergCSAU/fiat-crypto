...
(*In order to prove the correctness of Fp2_mul, some automation is in order
...
Proof.
  ...
  split; [auto|].
  split; [auto|]. split.
                    - repeat rewrite Prod.fst_pair; repeat rewrite Prod.snd_pair.
                    ...
                    - repeat rewrite Prod.fst_pair; repeat rewrite Prod.snd_pair.
                      ...
                      rewrite <- Z.mul_mod.
                        ...
                        rewrite <- (Zminus_mod ( _ * _)).
                      rewrite <- Zminus_mod. apply (f_equal (fun y => (y mod m))). ring.
Qed.

(*We can now write the tactics needed to automate proofs.*)
...
Proof.
  ...
  (*Fragmentation of the memorr after function body is necessary for stack allocation
  ...
  (*evars introduced and instantiated.
    The instantiation is done by Bedrock2's ecancel_assumption tactic.*)
  do 3 eexists; split; [|ecancel_assumption].

  (*It remains only to show the non-Bedrock2 specific parts of the postcondition.
    some is done by auto.*)
  split; [| auto].
